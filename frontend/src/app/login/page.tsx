    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative">
      ...
      <div className="relative z-10 w-full max-w-sm animate-fade-in">
        ...
        <div className="glass-strong gradient-border p-8 animate-scale-in">
          {walletAddress && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-500/[0.06] border border-emerald-500/15 mb-5">
              <Wallet className="w-3.5 h-3.5 text-emerald-400" />
              <span className="text-[11px] text-emerald-300 font-mono">{walletAddress.slice(0, 6)}...{walletAddress.slice(-4)}</span>
              <span className="text-[10px] text-emerald-500/60 ml-auto">Connected</span>
            </div>
          )}

          <h2 className="text-xl font-display font-bold text-white mb-1">Welcome back</h2>
          <p className="text-sm text-slate-500 mb-7">Access your compliance dashboard.</p>

          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="input-label">Email</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required autoFocus className="input-field" placeholder="admin@acmecorp.in" />
            </div>
            <div>
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full py-3 flex items-center justify-center gap-2">
              {loading ? (
                <><span className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full crypto-loader mr-2" />Signing in...</>
              ) : (
          ...
          <p className="text-center text-xs text-slate-600 mt-7">
            No account?{" "}
            <Link href="/register" className="text-indigo-400 hover:text-indigo-300 transition-colors">Register your organisation</Link>
          </p>
        </div>
      </div>
    </div>
