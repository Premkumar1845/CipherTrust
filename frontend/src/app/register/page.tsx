          ...
          {step === "account" ? (
            <form onSubmit={handleAccountSubmit} className="space-y-5">
              <div>
                ...
                <p className="text-sm text-slate-500">Set up your CipherTrust access credentials.</p>
              </div>

              <div>
                <label className="input-label">Full name</label>
                <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} required className="input-field" placeholder="Priya Sharma" />
              </div>
              <div>
                <label className="input-label">Email</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="input-field" placeholder="priya@acmecorp.in" />
              </div>
              <div>
                <label className="input-label">Password</label>
                <div className="relative">
                  <input type={showPw ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} className="input-field pr-10" placeholder="Min. 8 characters" />
                  <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 hover:text-slate-300 transition-colors">
              ...
              {error && <p className="text-xs text-red-400 bg-red-500/[0.08] border border-red-500/20 rounded-xl px-4 py-2.5">{error}</p>}

              <button type="submit" disabled={loading} className="btn-primary w-full py-3 flex items-center justify-center gap-2">
                {loading ? (
                  <><span className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full crypto-loader mr-2" />Creating account...</>
                ) : (
            ...
            <form onSubmit={handleOrgSubmit} className="space-y-5">
              <div>
                ...
                <p className="text-sm text-slate-500">This will be registered as your compliance entity.</p>
              </div>

              <div>
                <label className="input-label">Organisation name</label>
                <input type="text" value={orgName} onChange={(e) => setOrgName(e.target.value)} required className="input-field" placeholder="Acme Corp Pvt Ltd" />
              </div>
              ...
              <button type="submit" disabled={loading} className="btn-primary w-full py-3 flex items-center justify-center gap-2">
                {loading ? (
                  <><span className="w-4 h-4 border-2 border-indigo-400 border-t-transparent rounded-full crypto-loader mr-2" />Setting up...</>
                ) : (
